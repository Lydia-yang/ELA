...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, mask=True, **kwargs):
        super().__init__()
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, mask=False, mask_pro=None, **kwargs):
        ...
        if mask:
            B = x.shape[0]
            L = x.shape[1]
            labels = torch.ones_like(x[:,:,0])
            probability_matrix = torch.full(labels.shape, mask_pro)
            w, target = self.mask(x, targets=labels, probability_matrix = probability_matrix)
        ...
        if idx_to_group_img is None:
            if mask:
                return torch.cat([x_cls.transpose(1, 2), x], dim=1), target
            else:
    ...
    def mask(self, x, targets, masked_indices=None, probability_matrix=None):
        ...
        indices_replaced = torch.bernoulli(torch.full(targets.shape, 0.9)).bool() & masked_indices
        mask = indices_replaced.int()
